...
//#region node_modules/@metamask/multichain-ui/dist/esm/index-de0119fe.js
var NAMESPACE = "sdk-install-modal-web";
var BUILD = {
	allRenderFn: true,
...
var loadModule = (cmpMeta, hostRef, hmrVersionId) => {
	...
	if (!hmrVersionId || !BUILD.hotModuleReplacement) {
		...
		switch (bundleId) {
			case "mm-install-modal_2": return import(
				/* webpackMode: "lazy" */
				"./mm-install-modal_2.entry-CWi0m0Ym.js"
			...
			if (!appliedStyles.has(scopeId2)) {
				{
					...
					if (cmpMeta.$flags$ & 1 && styleContainerNode.nodeName !== "HEAD") styleContainerNode.insertBefore(styleElm, null);
				}
	...
	if (oldValue !== newValue) {
		...
		if (memberName === "class") {
			...
			for (const prop in oldValue) if (!newValue || newValue[prop] == null) {
				if (prop.includes("-")) elm.style.removeProperty(prop);
				else elm.style[prop] = "";
			}
			for (const prop in newValue) if (!oldValue || newValue[prop] !== oldValue[prop]) {
				if (prop.includes("-")) elm.style.setProperty(prop, newValue[prop]);
...
var createElm = (oldParentVNode, newParentVNode, childIndex, parentElm) => {
	...
	let elm;
	let childNode;
	if (newVNode2.$text$ !== null) elm = newVNode2.$elm$ = doc.createTextNode(newVNode2.$text$);
	else {
		if (!isSvgMode) isSvgMode = newVNode2.$tag$ === "svg";
		...
		if (isSvgMode && newVNode2.$tag$ === "foreignObject") isSvgMode = false;
		updateElement(null, newVNode2, isSvgMode);
		if (!!elm.getRootNode().querySelector("body") && BUILD.scoped && isDef(scopeId) && elm["s-si"] !== scopeId) elm.classList.add(elm["s-si"] = scopeId);
		if (newVNode2.$children$) for (i2 = 0; i2 < newVNode2.$children$.length; ++i2) {
			childNode = createElm(oldParentVNode, newVNode2, i2);
			if (childNode) elm.appendChild(childNode);
		}
		if (newVNode2.$tag$ === "svg") isSvgMode = false;
...
var connectedCallback = (elm) => {
	if ((plt.$flags$ & 1) === 0) {
		...
		} else if (hostRef == null ? void 0 : hostRef.$lazyInstance$) fireConnectedCallback(hostRef.$lazyInstance$);
...
var disconnectedCallback = async (elm) => {
	if ((plt.$flags$ & 1) === 0) {
		const hostRef = getHostRef(elm);
		if (hostRef == null ? void 0 : hostRef.$lazyInstance$) disconnectInstance(hostRef.$lazyInstance$);
		else if (hostRef == null ? void 0 : hostRef.$onReadyPromise$) hostRef.$onReadyPromise$.then(() => disconnectInstance(hostRef.$lazyInstance$));
	}
};
var bootstrapLazy = (lazyBundles, options = {}) => {
	...
	lazyBundles.map((lazyBundle) => {
		lazyBundle[1].map((compactMeta) => {
			...
			if (cmpMeta.$flags$ & 4) hasSlotRelocation = true;
			...
			const HostElement = class extends HTMLElement {
				...
				connectedCallback() {
					getHostRef(this);
					if (!this.hasRegisteredEventListeners) this.hasRegisteredEventListeners = true;
					if (appLoadFallback) {
	...
	if (cmpTags.length > 0) {
		if (hasSlotRelocation) dataStyles.textContent += SLOT_FB_CSS;
...
//#endregion
//#region node_modules/@metamask/multichain-ui/dist/esm/loader.js
var defineCustomElements = async (win, options) => {
	if (typeof window === "undefined") return void 0;
	await void 0;
	return bootstrapLazy([["mm-install-modal_2", [[
		1,
		"mm-install-modal",
		{
			"link": [1],
			"expiresIn": [2, "expires-in"],
			"showInstallModal": [4, "show-install-modal"],
...
//#endregion
//#region node_modules/@metamask/multichain-ui/dist/loader/index.js
...
//#endregion
export { registerInstance as a, defineCustomElements, h as i, createEvent as n, getElement as r, setNonce, loader_exports as t };
